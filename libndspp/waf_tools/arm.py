...
def setup_tool(conf, proc):
  ...
  arm_eabi='arm-eabi-'
  gcc = conf.find_program(arm_eabi+'gcc', path_list=[dka_bin])
  gxx = conf.find_program(arm_eabi+'g++', path_list=[dka_bin])
  if not gcc:
    conf.fatal('gcc was not found')
  if not gxx:
    conf.fatal('gxx was not found')

  v['CC'] = gcc
  v['CXX'] = gxx
  v['AR'] = '%sar'%arm_eabi
  v['RANLIB'] = '%sranlib'%arm_eabi
  v['LINK_CC'] = v['CC']
  ...
  v['CPPPATH'] = ['%s/libnds/include'%os.environ[DEVKITPRO]]
